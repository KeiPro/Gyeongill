...
HRESULT Missile::Init()
{
	...
	marble = new Image();
	if (FAILED( marble->Init("Image/구슬.bmp", size, size, true, RGB(255, 0, 255)) )) //HRESULT값에 따라서 얘가 성공했는지 실패 했는지를 반환하는 함수
	{
		MessageBox(g_hWnd, "Image/구슬.bmp 초기화 실패", "Missile.cpp", MB_OK); //윈도우 핸들, 
